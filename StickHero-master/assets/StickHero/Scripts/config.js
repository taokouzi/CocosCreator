...
window.Win_Global = 
{
    ...
    // 获取验证码
    get_code:function(tel,codeBtnLabel)
    {
    	if( !Win_Global.sendCode )
		{
	        if(!Win_Global.telReg.test(tel)) 
	        {
	            if( Win_Global.isWx == 1 ){ wx.showToast({title:'手机号码验证错误！（'+ tel +'）',icon:'none'}) }
	            else{console.log('手机号码验证错误！（'+ tel +'）')}
	        }
	    	else
	        {
	        	if( Win_Global.isWx == 1 ){wx.showLoading({title:'发送中...'})}
	        	Win_Global.sendCode = true;
	            Win_Global.ajaxPost(
	                Win_Global.path+'msg_validata.php',
	                {
	                    'action': 'get_code',
	                ...
	                function(res)
	                {
	                    ...
	                    {	
	                    	// console.log('发送成功');
	                    	if( Win_Global.isWx == 1 ){wx.showToast({title:'发送成功',icon:'success'})}
			                var times = 60;
	                    ...
	                    {
	                    	if( Win_Global.isWx == 1 ){wx.showToast({title:'号码重复',icon:'none'})}
	                        else{console.log('号码重复');}
	                    	Win_Global.sendCode = false;
	                    }
	                    else
	                    {
	                       if( Win_Global.isWx == 1 ){wx.showToast({title:'系统异常，请稍后再试哦',icon:'none'})}
	                       else{console.log('系统异常，请稍后再试哦');}
	                   	   Win_Global.sendCode = false;
	                    }
    ...
    // 绑定手机号
    bind_tel:function(tel,telLayer,code)
    {
        if(!Win_Global.telReg.test(tel)) 
        {
            if( Win_Global.isWx == 1 ){ wx.showToast({title:'手机号码验证错误！（'+ tel +'）',icon:'none'}) }
        ...
        {
            if( Win_Global.isWx == 1 ){ wx.showToast({title:'验证码错误！！（'+ code +'）',icon:'none'}) }
            else{console.log('验证码错误！！（'+ code +'）')}
            return false;
        }

        Win_Global.ajaxPost(
            Win_Global.path+'msg_validata.php', //task.php
            {
